...
from .path_utils import is_likely_binary, resolve_workspace_path
...
def register_read_workspace_text_file(mcp: FastMCP, workspace_root: Path) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "(1-based, inclusive). Omit both to read the entire file."
        ),
    )
    async def ws_read_text_file(
        ...
        end_line: Annotated[
            int,
            Field(description="Last line to return (1-based, inclusive). Defaults to end of file."),
        ] = 0,
    ) -> dict[str, Any]:
        try:
