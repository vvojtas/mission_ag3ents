...
async def _download_single_file(
    ...
    await hub.download_file(f"data/{hub.hub_api_key}/{name}", dest)
    return "downloaded"
...
def _build_hint(downloaded: list[str], skipped: list[str], errors: list[dict[str, str]]) -> str:
    ...
    if downloaded or skipped:
        parts.append("Files are under the MCP workspace root; read them locally as needed.")
    if errors:
...
def register_download_doc(mcp: FastMCP, settings: Settings, workspace_root: Path) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            "Download one or more files from the hub data path "
            "(/data/<your-api-key>/) and save them to the workspace. "
