...
def register_update_workspace_file(mcp: FastMCP, workspace_root: Path) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "Mode `delete` removes lines start_line through end_line (inclusive). "
            "Mode `replace` substitutes that range with new_content. "
            "Returns the full numbered file after the edit."
        ),
    )
    async def ws_update_file(
        ...
        mode: Annotated[
            Literal["delete", "replace"],
            Field(description='Edit mode: "delete" or "replace".'),
        ],
        start_line: Annotated[
            int | None,
            Field(description="1-based first line of the target range (required)."),
        ] = None,
        end_line: Annotated[
            int | None,
            Field(description="1-based inclusive last line of the target range (required)."),
        ] = None,
        new_content: Annotated[
            str | None,
            Field(description='Content to insert when mode is "replace".'),
        ] = None,
    ) -> dict[str, Any]:
        try:
        ...
        result = _apply_range_edit(mode, lines, start_line, end_line, new_content)
        if "error" in result:
            return result
...
def _apply_range_edit(
    ...
    """Compute the updated line list for delete or replace."""
